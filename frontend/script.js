...
// 현재까지 섭취한 영양소 값 (초기값)
let current = {
    calories: 1700, // 현재 칼로리 섭취량
    carbs: 100,     // 현재 탄수화물 섭취량(g)
    protein: 150,   // 현재 단백질 섭취량(g)
    fats: 200       // 현재 지방 섭취량(g)
};
...
document.getElementById("imageInput").addEventListener("change", function(event) {
    ...
    if (file) {                                       // 파일이 존재하면
        ...
        reader.onload = function(e) {                // 파일 읽기 완료 시 실행
...
async function uploadImage() {
    ...
    if (!fileInput.files.length) {                         // 파일 선택 안 했을 경우
...
function updateGoalsDisplay() {
    ...
    // 칼로리 값 및 체크박스 갱신
    goalItems[0].querySelector("span").textContent = 
        `kcal: ${current.calories} / ${goals.calories}`;               // 현재/목표 표시
    goalItems[0].querySelector("input").checked = 
        current.calories >= goals.calories;                            // 목표 달성 여부 체크

    // 탄수화물 값 및 체크박스 갱신
    goalItems[1].querySelector("span").textContent = 
        `carb.: ${current.carbs} / ${goals.carbs}`;
    goalItems[1].querySelector("input").checked = 
        current.carbs >= goals.carbs;

    // 단백질 값 및 체크박스 갱신
    goalItems[2].querySelector("span").textContent = 
        `prot.: ${current.protein} / ${goals.protein}`;
    goalItems[2].querySelector("input").checked = 
        current.protein >= goals.protein;

    // 지방 값 및 체크박스 갱신
    goalItems[3].querySelector("span").textContent = 
        `fat: ${current.fats} / ${goals.fats}`;
    goalItems[3].querySelector("input").checked = 
        current.fats >= goals.fats;
}
